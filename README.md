...
| Товары | `/products` | Каталог, цены, остатки FBO/FBS по складам, акции, закупочные цены |
...
В блоке «Внешние данные» профиля для каждого FBS-склада кабинета задаётся своя ссылка на
XLS/XLSX-файл с остатками (колонки **Баркод** и **Остаток FBS**):

- «Добавить склад FBS» → выбор склада из списка кабинета Ozon → ссылка на файл;
- настройки можно добавлять, изменять и удалять в любом количестве;
- регламентное задание обновляет остатки по каждому складу отдельно и выгружает их
  в Ozon по соответствующему `warehouse_id`;
- товары в архиве Ozon остатки не принимают (`Can't set positive stock to archived product`):
  такие позиции пропускаются без ошибки задания, их список виден в журнале и в деталях запуска,
  а повторная отправка выполняется только после следующей синхронизации товаров;
- на странице «Товары» значение в колонке «Остаток FBS» — суммарный остаток по всем
  складам; по клику открывается модалка с разбивкой «склад → остаток».
...
- Отчёт по остаткам
- Обновление остатков FBS по складам (из файлов, указанных в профиле)
