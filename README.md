...
| Заказы FBS | `/fbs-orders` | Новые и отправленные заказы FBS по складам, сборка заказа кнопкой «Собрать» |
...
Первоначальный план из ранних версий README **выполнен почти полностью**:

| Пункт плана | Статус |
|-------------|--------|
| Авторизация (login/register) | Готово |
| Blueprints страниц и REST API | Готово |
| Интеграция Ozon Seller API | Готово |
| Товары | Готово — каталог, синхронизация, акции, закупочные цены |
| Заказы | Готово — FBS/FBO, фильтры, маржа, экспорт |
| Заказы FBS | Готово — новые и отправленные заказы FBS по складам, сборка заказа в «Ожидает отгрузки» |
| Поставки | Готово — FBO, статусы, детали |
| Отчёты | Готово — остатки по складам, возвраты |
| Аналитика | Готово — планирование поставки, акции, склады и слоты |
| Регламентные задания | Готово — 5 заданий, настройка в интерфейсе |
| Уведомления | Готово — 4 типа событий |
| Журнал обновлений сервиса (`/changelog`) | Готово — release notes, редактирование для admin |

**Ещё не реализовано** (из старого плана «журнал с откатом»):

- **Откат изменений сущностей** — модель `ChangeLog` и `log_change()` пишут аудит (`old_value` / `new_value`), но UI и API для просмотра истории и отката нет.

Дополнительно: модели `Report` и `Analytics` в БД — задел под будущие отчёты; в интерфейсе пока не используются.
